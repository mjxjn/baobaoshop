...
if($_REQUEST['step'] == 'addexchange_to_cart'){
	/*------------------------------------------------------ */
	//-- 添加积分商品到购物车
	/*------------------------------------------------------ */
	...
	if ($_SESSION['user_id'] <= 0)
	{
		...
		$result['error']   = 1;
		$result['message'] = $_LANG['eg_error_login'];
	...
	" AND parent_id = 0 AND is_gift = 0 AND rec_type = '".CART_GENERAL_GOODS."'";
	...
	$sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
	" WHERE user_id = " . $_SESSION['user_id'] .
	" AND parent_id = 0 AND is_gift = 0 AND rec_type = '".CART_EXCHANGE_GOODS."'";
	
	if ($db->getOne($sql) != 0)
	{
		//show_message($_LANG['no_goods_in_cart'], '', '', 'warning');
		$result['error']   = 1;
		$result['message'] = "购物车中已有积分兑换商品";
		die($json->encode($result));
	}
	...
	/* 查询：取得兑换商品信息 */
	$goods = get_exchange_goods_info($goods->goods_id);
	if (empty($goods))
	{
		//ecs_header("Location: ./\n");
		//exit;
	...
	if($goods['goods_number'] == 0 && $_CFG['use_storage'] == 1)
	{
		...
		$result['error'] = 1;
		$result['message'] = $_LANG['eg_error_number'];
		die($json->encode($result));
	}
	/* 查询：检查兑换商品是否是取消 */
	if ($goods['is_exchange'] == 0)
	{
		...
		$result['error'] = 1;
		$result['message'] = $_LANG['eg_error_status'];
		die($json->encode($result));
	}
	
	$user_info   = get_user_info($_SESSION['user_id']);
	$user_points = $user_info['pay_points']; // 用户的积分总数
	$sql = "select goods_price from ". $ecs->table('cart') ." where user_id=".$_SESSION['user_id'].
	" AND parent_id = 0 AND is_gift = 0 AND rec_type = '$flow_type'";
	$res = $db->query($sql);
	while ($row = $db->fetchRow($res))
	{
		$exchange_integral += $row['goods_price'] ;
	}
	
	if (($goods['exchange_integral']+$exchange_integral) > $user_points)
	{
		//show_message($_LANG['eg_error_integral'], array($_LANG['back_up_page']), array($back_act), 'error');
		$result['error'] = 1;
		$result['message'] = $_LANG['eg_error_integral'];
		die($json->encode($result));
	}
	/* 查询：取得规格 */
	$specs = '';
	...
	$cart = array(
			...
			'goods_price'    => $goods['exchange_integral'],
			'goods_number'   => $number,
...
elseif ($_REQUEST['step'] == 'login')
{
    ...
    else
    {
        ...
        {
			$sql = "select user_name from ".  $ecs->table('users') . " where user_name = '" .$_POST['username']. "' or email = '".$_POST['username']. "' or mobile_phone ='" .$_POST['username']."'" ;
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if (!check_consignee_info($consignee, $flow_type))
    {
		...
		$smarty->assign('m_consignee', $m_consignee);
    }else{
		$m_consignee=1;
		$smarty->assign('m_consignee', $m_consignee);
		
		$m_province=$consignee['province'];
		$m_city=$consignee['city'];
		$m_district=$consignee['district'];
		
		$sql="SELECT region_name from " . $ecs->table('region') . " where region_id = " . $m_province;
		$m_province = $db->getOne($sql);
		$sql="SELECT region_name from " . $ecs->table('region') . " where region_id = " . $m_city;
		$m_city = $db->getOne($sql);
		$sql="SELECT region_name from " . $ecs->table('region') . " where region_id = " . $m_district;
		$m_district = $db->getOne($sql);
		
		$smarty->assign('m_province', $m_province);
		$smarty->assign('m_city', $m_city);
		$smarty->assign('m_district', $m_district);
	}
    ...
    $smarty->assign('e_total', $cart_goods['total']);
    ...
    $payment_list = available_payment_list($cod, $cod_fee);
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
		...
		$support_cod = $shipping_info['support_cod'];
		$payment_list=available_payment_list(1,$result['cod_fee']);
		if(isset($payment_list))
		{
			foreach ($payment_list as $key => $payment)
			{
		...
		foreach ($payment_list as $key => $payment)
		{
			...
			if($payment['pay_code']=="alipay"){
				$payment_name='<img src="themes/yingge/images/onlinepay_5.gif" width=133 height=39 border=0 />';
			}elseif($payment['pay_code']=="tenpay"){
				$payment_name='<img src="themes/yingge/images/onlinepay_29.gif" width=133 height=39 border=0 />';
			}elseif($payment['pay_code']=="ips"){
				/*$MerCode = "016921";
			    $MerKey = "4021-1725-0042-8710";
				$md5Str = strtolower(md5($MerCode.$MerKey));
				$objSoapClient = new SoapClient("http://webservice.ips.com.cn/web/Service.asmx?WSDL"); 
							$param = array(
								'MerCode' => $MerCode,
								'SignMD5' => $md5Str
								);
				$data = $objSoapClient->GetBankList($param);
				$result = $data->GetBankListResult;
				$payment_name=$result;*/
			}else{
				$payment_name=$payment['pay_name'];
			}
			if($payment['is_cod'] != $support_cod){
				
			}else{
				$payment_content.='<div>
				<label for="radio_'.$payment['pay_id'].'">
				<input type="radio" name="payment" value="'.$payment['pay_id'].'" '.$checked.' isCod="'.$payment['is_cod'].'" onclick="selectPayment(this)"/>
		...
		$result['pay_info'] = '<h1 class="cinfo_subt">付款方式</h1>
<div class="g_line"> 
            <div id="bank_box">
    <div class="bank_box"> 
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    else
    {
		...
		if($result['pay_code']!='balance')
		{
			...
			$order['surplus']=$_SESSION['flow_order']['surplus'];
		}
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    if ($_CFG['sms_order_placed'] == '1')
    {
		...
		$smarty->assign('oid',      $order['order_sn']);
...
elseif($_REQUEST['step'] == 'act_send_sms')
{
	...
	*/	
	...
	$msg="婴格母婴商城提醒您，您的订单(订单号：".$order_sn.")已提交成功。".$sms_site_info['order_charge'];
...
elseif ($_REQUEST['step'] == 'add_favourable')
{
    ...
    if ($favourable['act_type'] == FAT_GOODS)
    {
        ...
        /* 检查数量是否超过上限 */
		$gift_number=$_POST['goods_number'];
		...
		if($gift_number[$act_id] > $favourable['act_type_ext'])
		{
			show_message($_LANG['gift_count_exceed']);
		}
        ...
        {
			if($gift_number[$gift['id']]>$favourable['act_type_ext'])
			{
				show_message($_LANG['gift_count_exceed']);
			}
            if (in_array($gift['id'], $_POST['gift']))
            {
                add_gift_to_cart($act_id, $gift['id'], $gift['price'], $gift_number[$gift['id']]);
            }
        }
    }
...
else
{
    ...
    {
        // 取得用户可用红包
        $user_bonus = user_bonus($_SESSION['user_id'], $total['goods_price'], $cart_goods);
        if (!empty($user_bonus))
        {
			$bonus_list_num=0;
            foreach ($user_bonus AS $key => $val)
            ...
            $smarty->assign('bonus_list', $user_bonus);
			$smarty->assign('bonus_list_num', $bonus_list_num);
        }else{
			$smarty->assign('bonus_list_num', 0);
		}
	...
	foreach($favourable_list as $key => $val)
	{
		...
		if($favourable_list[$key]['available'])
		{
			$smarty->assign('youhui_show', $favourable_list[$key]['available']);
		}
	}
	if($total['favourable_amount'] >= $min_amount)
	{
		$smarty->assign('is_favourable',1);
		$is_favourable=1;
		$sql = "SELECT is_gift  FROM " . $GLOBALS['ecs']->table('cart') .
				" WHERE session_id = '" . SESS_ID . "' AND rec_type in ('" . CART_GENERAL_GOODS . "','".CART_EXCHANGE_GOODS."')" .
				" ORDER BY rec_id asc";
	...
	else
	{
		...
		$smarty->assign('m_amount', $m_amount);
		$smarty->assign('m_amount_s', $m_amount_s);
...
function favourable_list($user_rank,$price_amount=0)
{
    ...
    while ($favourable = $GLOBALS['db']->fetchRow($res))
    {
        ...
        foreach ($favourable['gift'] as $key => $value)
        {
			...
			if(empty($favourable['gift'][$key]['goods_thumb']))
			{
				$favourable['gift'][$key]['goods_thumb']="images/no_picture.gif";
			}
 ...
 */
function add_gift_to_cart($act_id, $id, $price, $goods_number)
{
 ...
 * @return  array
 */
function get_recommend_rand()
{
	$sql = 'SELECT g.goods_id,g.brand_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price,b.brand_name, ' .
				...
				"where g.is_on_sale=1 and g.goods_number > 0 and g.is_alone_sale=1 ".
				"order by rnd limit 0, 12 ";
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        else
        {
            ...
            $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                                               sub_str($row['goods_name'], 10) : $row['goods_name'];
			...
			$type_goods['rand'][] = $goods[$idx];
    }
	return $type_goods['rand'];
}
