...
if ($_REQUEST['act'] == 'list')
{
    ...
    if (!$smarty->is_cached('exchange.dwt', $cache_id))
    {
        ...
        $count = get_exchange_goods_count($children, $integral_min, $integral_max, $ext, $cat_id);
        $max_page = ($count> 0) ? ceil($count / $size) : 1;
        if ($page > $max_page)
        {
            $page = $max_page;
        }
        $goodslist = exchange_get_goods($children, $integral_min, $integral_max, $ext, $size, $page, $sort, $order, $cat_id);
        if($display == 'grid')
        {
        ...
        $smarty->assign('goods_type_id',         $cat_id);
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    {
        show_message($_LANG['eg_error_number2'], array($_LANG['back_up_page']), array($back_act), 'error');
    }*/
 ...
 * @return  integer
 */
function get_exchange_goods_count($children, $min = 0, $max = 0, $ext='', $cat_id)
{
