...
$not_login_arr =
array('login','act_login','register','act_register','act_register_pass','register_address','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer','act_register_ok','is_tel_registered','check_tel','act_send_sms','send_pwd_email','reset_send_sms','check_send_email');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'edit_address','collection_list', 'integration',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'act_register_pass','comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','lg_act_registersuccess','ck_code','register_address','act_send_sms','send_pwd_email','reset_send_sms','check_send_email');
...
if (in_array($action, $ui_arr))
{
    ...
    $ur_here_name = '';
    if($action==""){
    	 $ur_here_name=$_LANG['user_center'];
    }
    if($action=="order_list"||$action=="order_detail"){
    	$ur_here_name="我提交的订单";
    }
    ...
    if($action=="address_list"||$action=="edit_address"){
    	$ur_here_name="我的地址薄";
    }
    ...
    $position = assign_ur_here(0,$ur_here_name);
...
if ($action == 'default')
{
    ...
    $goods_top10_data = get_recommend_goods('hot');
...
/* 手机验证码发送 */
elseif($action == 'act_send_sms')
{
	$other['mobile_phone'] = isset($_POST['mobile']) ? $_POST['mobile'] : '';
	//echo $other['mobile_phone'];
	include(dirname(__FILE__) . '/admin/includes/cls_sms.php');   
    $sql = 'SELECT `serialNumber`, `password`, `smsnum`, `sessionKey`, `register_charge`
	...
	*/	
	...
	$msg="婴格母婴商城提醒您，您的短信验证码为：".$rand_code.",".$sms_site_info['register_charge'];

		
			$result=send_sms($other['mobile_phone'],$msg);
			if($result!=null && $result=="0")
			{
				
			}else
			{
				show_message("发送失败，请联系管理员！");
				exit;
			}
...
elseif ($action == 'act_register')
{
	...
	$baby=isset($_POST['baby']) ? trim($_POST['baby']) : '';
	$babybirthday=$_POST['StartDateYear']."-".$_POST['StartDateMonth']."-".$_POST['StartDateDay'];
	
		

	if($_POST['is_key']==1)
	{
		$keynumber=isset($_POST['keynumber']) ? trim($_POST['keynumber']) : '';
	}
	
    $back_act = isset($_POST['back_act']) ? trim($_POST['back_act']) : '';
    
    	$username=str_check($username);

		$password=str_check($password);

		$email=str_check($email);
		
		$other['mobile_phone']=str_check($other['mobile_phone']);
		
		$baby=verify_id($baby);
		
		$babybirthday=str_check($babybirthday);
		
		$keynumber=str_check($keynumber);
		
		$back_act=str_check($back_act);

	$sql = "select mobile_phone from ". $ecs->table('users') . " where mobile_phone =" .$other['mobile_phone'];
	if ($db -> getOne($sql))
	{
		show_message("此手机号码已被注册");
		exit;
	}
	...
	$smarty->assign('msn', $other['msn']);
	$smarty->assign('qq', $other['qq']);
	$smarty->assign('office_phone', $other['office_phone']);
	$smarty->assign('home_phone', $other['home_phone']);
	$smarty->assign('sel_question', $sel_question);
	$smarty->assign('passwd_answer', $passwd_answer);
	$smarty->assign('mobile', $other['mobile_phone']);
	$smarty->assign('baby', $baby);
	$smarty->assign('babybirthday', $babybirthday);
	$smarty->assign('keynumber', $keynumber);
	$smarty->assign('back_act', $back_act);
	$smarty->assign('rand_code', $rand_code);
	$smarty->assign('helps',      get_shop_help());        // 网店帮助
	$smarty->assign('action',     'register_address');
	$smarty->display('user_passport.dwt');
}

elseif($action == 'register_address')
{
	...
	$baby = isset($_POST['baby']) ? trim($_POST['baby']) : '';
	$babybirthday = isset($_POST['babybirthday']) ? trim($_POST['babybirthday']) : '';
	$keynumber = isset($_POST['keynumber']) ? trim($_POST['keynumber']) : '';
...
elseif($action == 'act_register_pass')
{
    ...
    else
    {
		...
		$other['mobile_phone'] = isset($_REQUEST['mobile']) ? $_REQUEST['mobile'] : '';
	
	    $back_act = "";
	    
		$other['baby'] = isset($_REQUEST['baby']) ? trim($_REQUEST['baby']) : '';
		$other['babybirthday'] = isset($_REQUEST['babybirthday']) ? trim($_REQUEST['babybirthday']) : '';
		$other['keynumber'] = isset($_REQUEST['keynumber']) ? trim($_REQUEST['keynumber']) : '';
		...
		$other['baby']=verify_id($other['baby']);
		
		$babybirthday=str_check($babybirthday);
		
		$other['keynumber']=str_check($other['keynumber']);
		...
		*
		...
		$sql = ' select COUNT(*) AS ips FROM ' . $ecs->table('users') . ' WHERE last_ip = "'.real_ip().'"';
		$ips=$GLOBALS['db']->getOne($sql);
		if($ips != 0){
		...
		**/
        if (register($username, $password, $email, $other) !== false)
        {
			...
			//ecs_header("Location: user.php?act=check_send_email&email=".$email."\n");
...
elseif ($action == 'check_send_email')
{
			$email    = isset($_REQUEST['email']) ? trim($_REQUEST['email']) : '';
			$email=str_check($email);
			if(empty($email)){
			...
			$a_emailurl=explode("@",$email);
			$emailurl=$a_emailurl[count($a_emailurl)-1];
			if($emailurl=="qq.com"||$emailurl=="vip.qq.com"){
				$emailurl="mail.qq.com";
			}elseif($emailurl=="sina.com"||$emailurl=="vip.sina.com"){
				$emailurl="mail.sina.com.cn";
			}elseif($emailurl=="gmail.com"){
				$emailurl="accounts.google.com";
			}
			$smarty->assign('email', $email);
			$smarty->assign('emailurl', $emailurl);
			$smarty->assign('helps',      get_shop_help());        // 网店帮助
			$smarty->display('user_passport.dwt');
}
/* 送货地址添加 + 注册会员的处理 */
elseif ($action == 'act_register_ok')
{
	include_once(ROOT_PATH . 'includes/lib_passport.php');
    ...
    else
    {
		...
		{
	 		show_message("手机验证码错误", "请重新注册", '/user.php?act=register');	
	 		exit;		
		}*/
		/**
		* 同一ip不能重复注册2012.6.7
		...
		**/
	    ...
	    $other['office_phone'] = isset($_POST['office_phone']) ? $_POST['office_phone'] : '';
	    $other['home_phone'] = isset($_POST['home_phone']) ? $_POST['home_phone'] : '';
	    $other['mobile_phone'] = isset($_POST['mobile']) ? $_POST['mobile'] : '';
	    $sel_question = empty($_POST['sel_question']) ? '' : $_POST['sel_question'];
	    $passwd_answer = isset($_POST['passwd_answer']) ? trim($_POST['passwd_answer']) : '';
		...
		$other['baby'] = isset($_POST['baby']) ? trim($_POST['baby']) : '';
		$other['babybirthday'] = isset($_POST['babybirthday']) ? trim($_POST['babybirthday']) : '';
		$other['keynumber'] = isset($_POST['keynumber']) ? trim($_POST['keynumber']) : '';
...
/* 验证用户手机号码是否被注册 */
//elseif($_GET['action'] == 'aaa')
elseif($action == 'check_tel')
{
    $mobile = trim($_GET['mobile_nub']);
    if ($user->check_tel($mobile))
    {
        echo 'false';
    }
    else
...
elseif ($action == 'act_login')
{
	...
	$sql = "select user_name from ".  $ecs->table('users') . " where user_name = '" .$username. "' or email = '".$username. "' or mobile_phone ='" .$username."'" ;
...
elseif($action == 'integration')
{
	...
	$account_type = isset($_REQUEST['account_type']) ? $_REQUEST['account_type'] : 'pay_points';
    ...
    $pager  = get_pager('user.php', array('act' => 'integration','account_type' =>'pay_points'), $record_count, $page);
	...
	//$account_list = get_accountlist($user_id,'pay_points',$page);
	
	$smarty->assign('categories',       get_categories_tree()); // 分类树
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    $smarty->assign('account_list', $arr);
    $smarty->assign('pager',  $pager);
    $smarty->assign('type',  $type);
        $smarty->assign('info',        get_user_default($user_id));
	$smarty->display('user/integration.html');
...
$smarty->assign('categories',       get_categories_tree()); // 分类树
    ...
    $smarty->display('user/profile.html');
}
/* 修改个人资料的处理 */
...
//    $email     = !empty($_POST['email'])     ? trim($_POST['email'])     : '';
    ...
    //用户名和邮件地址是否匹配
    $user_info = $user->check_tel($mobile_phone);
    if ($user_info)
    {
		$smarty->assign('mobile', $mobile_phone);
		$smarty->display('user_passport.dwt');
    ...
    {
        //用户名与邮件地址不匹配
        show_message("对不起，没有该手机号的会员信息，请确认后再次提交！");
    }
...
{	
	...
	$client = new Client($gwUrl,$serialNumber,$password,$sessionKey,$proxyhost,$proxyport,$proxyusername,$proxypassword,$connectTimeOut,$readTimeOut); 
	$new_password = rand(100000,999999);//生成手机验证码
	$msg="婴格母婴商城提醒您，您的新密码为：".$new_password.",建议您登陆后进入会员中心重新修改密码,如有任何问题，请咨询在线客服[婴格]";
	...
	$old_password=null;
			...
			$code = md5($user_id . $_CFG['hash_code'] . $user_info['reg_time']);
			if($result!=null && $result=="0")
			{
				//$sql = "update ". $ecs->table('users') ." set password ='" .$new_password."' where mobile_phone ='" .$mobile_phone."'";
				//$db->query($sql);
				
				if (($user_info && (!empty($code) && md5($user_id . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    {
...
elseif ($action == 'reset_password')
{
    ...
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    //显示重置密码的表单
    $smarty->display('user/resetpassword.html');
}
...
elseif ($action == 'act_edit_password')
{
    ...
    $old_password=str_check($old_password);
    $new_password=str_check($new_password);
    $user_id=verify_id($user_id);
...
elseif ($action == 'order_list')
{
    ...
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
    $pay_status = isset($_REQUEST['paystatus']) ? intval($_REQUEST['paystatus']) : '';
    $shipping_status = isset($_REQUEST['shippingstatus']) ? intval($_REQUEST['shippingstatus']) : '';
    $order_status = isset($_REQUEST['orderstatus']) ? intval($_REQUEST['orderstatus']) : '';

    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE user_id = '$user_id'");
    $all_record_count = $record_count;
    if($pay_status===1){
    	$record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE order_status<1 and pay_status =0 and user_id = '$user_id'");
    }
    $pay_status_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE order_status<1 and pay_status =0 and user_id = '$user_id'");
    if($shipping_status===1){
    	$record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE order_status =1 and shipping_status in (1,3,4,5) and user_id = '$user_id'");
    }
    $shipping_status_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE order_status =1 and shipping_status in (1,3,4,5) and user_id = '$user_id'");
    if($order_status===1){
    	$record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE shipping_status =2 and user_id = '$user_id'");
    	
    }
    $order_status_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE shipping_status =2 and user_id = '$user_id'");

    $pager  = get_pager('user.php', array('act' => $action,'paystatus'=>$pay_status,'shippingstatus'=>$shipping_status,'orderstatus'=>$order_status), $record_count, $page);

    $orders = get_user_orders($user_id, $pager['size'], $pager['start'],$pay_status,$shipping_status,$order_status);
    ...
    $smarty->assign('all_record_count',  $all_record_count);
    $smarty->assign('pay_status_count',  $pay_status_count);
    $smarty->assign('shipping_status_count',  $shipping_status_count);
    $smarty->assign('order_status_count',  $order_status_count);
    ...
    $smarty->assign('categories',       get_categories_tree()); // 分类树
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    $smarty->display('user/orderlist.html');
}
...
elseif ($action == 'order_detail')
{
    ...
    /* 订单商品 */
    $goods_list = order_goods($order_id, $user_id);
    ...
    foreach($province_list as $key => $val){
    	if($val['region_id']==$order['province']){
    		$province=$val['region_name'];
    	}
    }
    foreach($city_list as $key => $val){
    	if($val['region_id']==$order['city']){
    		$city=$val['region_name'];
    	}
    }
    foreach($district_list as $key => $val){
    	if($val['region_id']==$order['district']){
    		$district=$val['region_name'];
    	}
    }
    ...
    $smarty->assign('categories',       get_categories_tree()); // 分类树
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    $smarty->display('user/orderdetail.html');
}
...
elseif ($action == 'address_list')
{
    ...
    //$smarty->display('user_transaction.dwt');
    $smarty->display('user/addresslist.html');
}
elseif($action == 'edit_address'){
    ...
    if (!empty($result))
    {
        while ($temp_arr = $db->fetchRow($result))
        {
                /*$addresssinfo['serialNumber'] = $temp_arr['serialNumber'];
				$addresssinfo['password'] = $temp_arr['password'];
				$addresssinfo['smsnum'] = $temp_arr['smsnum'];
				$addresssinfo['sessionKey'] = $temp_arr['sessionKey'];
				$addresssinfo['register_charge'] = $temp_arr['register_charge'];*/
        	$addresssinfo=$temp_arr;
        	$province_list[] = get_regions(1, $addresssinfo['country']);
		     $city_list[]     = get_regions(2, $addresssinfo['province']);
		     $district_list[] = get_regions(3, $addresssinfo['city']);
        }
    }
    ...
    $smarty->assign('province_list',    $province_list[0]);
    $smarty->assign('city_list',        $city_list[0]);
    $smarty->assign('district_list',    $district_list[0]);
    $smarty->assign('integral_scale',   $_CFG['integral_scale']);
    $smarty->assign('name_of_region',   array($_CFG['name_of_region_1'], $_CFG['name_of_region_2'], $_CFG['name_of_region_3'], $_CFG['name_of_region_4']));
    $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
    $smarty->assign('address',          $addresssinfo);
    $smarty->assign('info',        get_user_default($user_id));
    $smarty->display('user/editaddress.html');
...
//验证用户手机号
elseif ($action == 'lg_act_edit_address')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    include_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/shopping_flow.php');
    $smarty->assign('lang', $_LANG);
...
if(!empty($address['mobile'])){
		 $sql = 'UPDATE ' . $ecs->table('users') . " SET `mobile_phone`='{$address['mobile']}', `is_validated`=1  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
        $db->query($sql);
		}
		
    if (update_address($address))
    {
		header('Location:user.php?act=lg_act_registersuccess');
    }
	
	
}
elseif($action == 'lg_act_registersuccess'){
	
	
	  $smarty->display('user_clips.dwt');
	
	}
elseif($action == 'lg_act_edit_profile'){
	  $badyday = trim($_POST['babyyear']) .'-'. trim($_POST['babymonth']) .'-'. trim($_POST['babyday']);
	  $baby=!empty($_POST['baby'])?$_POST['baby']:0;
	 $sql = 'UPDATE ' . $ecs->table('users') . " SET `baby`='{$baby}', `babybirthday`='{$badyday}'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
        $db->query($sql);
		
	 show_message("您的信息已成功提交", "返回会员中心", 'user.php');
	}	
/* 删除收货地址 */
...
elseif ($action == 'collection_list')
{
    ...
    $smarty->assign('categories',       get_categories_tree()); // 分类树
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    $smarty->display('user/collectionlist.html');
}
...
elseif ($action == 'comment_list')
{
    ...
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    //$smarty->display('user_clips.dwt');
    $smarty->display('user/commentlist.html');
}
...
elseif ($action == 'booking_list')
{
    ...
    $smarty->assign('categories',       get_categories_tree()); // 分类树
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    $smarty->display('user/bookinglist.html');
}
/* 添加缺货登记页面 */
elseif ($action == 'add_booking')
{
    ...
    $smarty->assign('categories',       get_categories_tree()); // 分类树
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    $smarty->display('user/addbooking.html');
...
elseif ($action == 'bonus')
{
    ...
    $smarty->assign('categories',       get_categories_tree()); // 分类树
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    $smarty->display('user/bonus.html');
}
...
elseif ($action == 'affiliate')
{
    ...
    $smarty->assign('categories',       get_categories_tree()); // 分类树
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
    $smarty->display('user/affiliate.html');
}
...
{
	global $client;
	$statusCode = $client->sendSMS(array($mobiles),$content,$sendTime,$addSerial='UTF-8');
	return $statusCode;
}
/**
	* 登录 用例
