<?php
			...
			$hash_code = isset($_POST['hash_code']) && trim($_POST['hash_code']) ? trim($_POST['hash_code']) : '';
			 if (!$admin_user || !$hash_code) {
			    echo '107';
			    exit;
			}
			$sql = "SELECT user_id, user_name, password, action_list, last_login FROM " . $ecs->table('admin_user') . " WHERE user_name = '" . $admin_user. "'";
			$row = $db->getRow($sql);
			if (!$row) {
			    echo '106';
			    exit;
			}
			$server_hash = md5($row['user_name'].substr($row['password'], 0, 6));
			if ($server_hash!=$hash_code) {
			    echo '101';
			    exit;
			}
