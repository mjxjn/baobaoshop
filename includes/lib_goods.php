 ...
 * @return  array
 */
function get_top10_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                $cat_arr[$row['cat_id']]['url']  = build_uri('top10', array('cid' => $row['cat_id']), $row['cat_name']);
 ...
 * @return  array
 */
function get_top10($cats = '',$num = '',$acats = '')
{
	if(!empty($acats)){
		$arrcats = explode(",",$acats);
		$where =" and 1 ";
		foreach($arrcats as $val){
		    $cats = get_children($val);
		    $where .= !empty($cats) ? "OR ($cats OR " . get_extension_goods($cats) . ") " : '';
		}
	}else{
		$cats = get_children($cats);
		$where = !empty($cats) ? "AND ($cats OR " . get_extension_goods($cats) . ") " : '';
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++)
    {
		...
		$arr[$i]['wvera_rank']  = $arr[$i]['co_number']!=0 ? number_format(($arr[$i]['co_rank'] / $arr[$i]['co_number'])*20,0,'.','') : 100;
		if ($arr[$i]['promote_price'] > 0)
        {
            $promote_price = bargain_price($arr[$i]['promote_price'], $arr[$i]['promote_start_date'], $arr[$i]['promote_end_date']);
        }
		...
		{
		$arr[$i]['zhekou']      =($promote_price > 0) ? number_format($promote_price/$arr[$i]['market_price'],2,'.','')*10 : number_format($arr[$i]['shop_price']/$arr[$i]['market_price'],2,'.','')*10;
		}else
		{
			$arr[$i]['zhekou']=0;
		}
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
		...
		$type_goods['rand'] = array();
        $data = read_static_cache('recommend_goods');
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order, RAND() AS rnd  ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
 ...
 * @return  array
 */
function get_promote_goods_count($cats = '')
{
	$time = gmtime();
...
function get_rand_goods($size){
	/* 取得促销lbi的数量限制 */
	//$num = get_library_number("recommend_promotion");
	$sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
			"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
			"promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img,g.original_img, " .
	...
	{
		$cx_sql='SELECT count(*) FROM' .$GLOBALS['ecs']->table('order_goods') .' where goods_id='. $row['goods_id'];
		if ($GLOBALS['db']->getOne($cx_sql))
		{
			$goods[$idx]['count'] = $GLOBALS['db']->getOne($cx_sql);
		}
		else
		{
			$goods[$idx]['count'] = 0;
...
function get_promote_goods($cats = '',$size=4, $page=1)
{
    $time = gmtime();
    $order_type = $GLOBALS['_CFG']['recommend_order'];
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .
            " AND g.is_promote = 1 AND promote_start_date <= '$time' AND promote_end_date >= '$time' ".$where;
    $sql .= $order_type == 1 ? ' ORDER BY g.sort_order, g.last_update DESC' : ' ORDER BY rnd';
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if ($row['promote_price'] > 0)
        {
			...
			{
				$goods[$idx]['zhekou']        = number_format($row['promote_price']/$row['market_price'],2,'.','')*10;
			}else
			{
				$goods[$idx]['zhekou']        = 0;
			}
			$goods[$idx]['jiesheng']        = price_format($row['market_price']-$row['promote_price']);
			$goods[$idx]['promote_start_date'] = local_date('Y-m-d H:i:s', $row['promote_start_date']);
			$goods[$idx]['promote_end_date'] = local_date('D, d M Y H:i:s O', $row['promote_end_date']);
        }
        else
        {
            $goods[$idx]['promote_price'] = '';
 ...
 * @return  array
 */
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='', $num = '0')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.brand_id, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.cat_name,c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
    ...
    if ($row !== false)
    {
		...
		if($row['market_price']!=0)
		{
		$row['zhekou']        = number_format($row['shop_price']/$row['market_price'],2,'.','')*10;
		}else
		{
			$row['zhekou']        =0;
		}
		$row['jiesheng']        = price_format($row['market_price']-$row['shop_price']);
		$row['mmarket_price']        = $row['market_price'];
...
{
	$sql = "SELECT c.*, g.is_promote, g.promote_start_date, g.promote_end_date, g.promote_price " .
            " FROM " . $GLOBALS['ecs']->table('cart') . " c LEFT JOIN " . $GLOBALS['ecs']->table('goods') ." g on c.goods_id = g.goods_id " .
            " WHERE c.session_id = '" . SESS_ID . "' AND c.rec_type = '" . CART_GENERAL_GOODS . "' AND c.is_gift = 0";
    $res = $GLOBALS['db']->query($sql);
	
	while ($mrow = $GLOBALS['db']->fetchRow($res))
    {
		if($mrow['promote_price']>0 && $mrow['is_promote'] >0 )
		{
			if ($mrow['promote_price'] == 0)
			{
				$total += $mrow['goods_price'] * $mrow['goods_number'];
			}
			else
			{
				$time = gmtime();
				if ($time >= $mrow['promote_start_date'] && $time <= $mrow['promote_end_date'])
				{
	...
	$sql = "SELECT * " .
            ...
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "' AND is_gift > 0";
    $res = $GLOBALS['db']->query($sql);
	while ($mrow = $GLOBALS['db']->fetchRow($res))
    {
		$m_sql = "SELECT min_amount FROM " . $GLOBALS['ecs']->table('favourable_activity') . " WHERE act_id = ".$mrow['is_gift'];
		$m_f = $GLOBALS['db']->getOne($m_sql);
		$favourable_total += $m_f * $mrow['goods_number'];
	}
	
	$favourable_total=$total-$favourable_total;
    ...
    if (!empty($row))
    {
		...
		if($row['min_amount'] != 0 && $favourable_total > 0)
		{
