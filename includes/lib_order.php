...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
	...
	{
		$sql .= 'AND is_cod = 1 '; // 如果支持货到付款
	}
    if ($is_online)
    {
        $sql .= "AND is_online = '1' ";
    }
    $sql .= 'ORDER BY pay_id'; // 排序
 ...
 * @return  array   订单信息（金额都有相应格式化的字段，前缀是formated_）
 */
function order_info_user($order_id, $order_sn = '')
{
	...
	if ($order)
	{
		$sql="select is_cod from ".$GLOBALS['ecs']->table('payment') .
		...
		$sql="SELECT * FROM " .$GLOBALS['ecs']->table('order_action') .
		" WHERE order_id=".$order['order_id']." order by action_id asc";
		$res = $GLOBALS['db']->query($sql);
		while ($row = $GLOBALS['db']->fetchRow($res))
		{
			if($row['shipping_status']==5){
				$order['outbound_time'] =local_date("Y-m-d H:i:s",$row["log_time"]);
			}
			if($row['shipping_status']==1||$row['shipping_status']==2){
				$order['deal_time'] = local_date("Y-m-d H:i:s",$row["log_time"]);
			}
			if($row['order_status']==OS_CANCELED){
				$order['cancel_time']= local_date("Y-m-d H:i:s",$row["log_time"]);
				$order['paytype']=3;
			}
		}
		if($is_cod==1){
			if($order['order_status'] == OS_CANCELED){
				...
				if($order['order_status']==OS_UNCONFIRMED){
					$order['order_status'] = "等待配货";
					$order['handler']="<a href=\"user.php?act=cancel_order&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\" target='_blank'><img src='/themes/yingge/user/images/cancelbtn.jpg' width='92' height='30' border='0' /></a>";
					$order['pay_step']=1;
				}
				else if($order['order_status'] == OS_CONFIRMED){
					$order['order_status'] = "开始配货";
					$order['pay_step']=2;
				}
				else if ($order['order_status'] == OS_SPLITED)
				{
					/* 对配送状态的处理 */
					if ($order['shipping_status'] == SS_SHIPPED || $order['shipping_status'] ==SS_SHIPPED_ING || $order['shipping_status'] ==SS_PREPARING || $order['shipping_status'] ==SS_SHIPPED_PART)
					{
						$order['order_status'] = "已发货";
						$order['pay_step']=3;
						@$order['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\" target='_blank'>".$GLOBALS['_LANG']['received']."</a>";
					}
			...
			if($order['order_status'] == OS_CANCELED){
				...
				if ($order['pay_status'] == PS_UNPAYED)
				{
					...
					@$order['handler'] = $order['pay_online']."&nbsp;&nbsp;<a style='float:right;' href=\"user.php?act=cancel_order&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\" target='_blank'><img src='/themes/yingge/user/images/cancelbtn.jpg' width='92' height='30' border='0' /></a>";
					//}else{
						//@$order['handler'] ="<a style='float:right;' href=\"user.php?act=cancel_order&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\" target='_blank'><img src='/themes/yingge/user/images/cancelbtn.jpg' width='92' height='30' border='0' /></a>";
					//}
				}
...
{
    $sql = "SELECT g.rec_id, g.goods_id, g.goods_name, g.goods_sn,gg.goods_thumb, g.market_price, g.goods_number, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $sql = "select count(comment_id) as cnum from ".$GLOBALS['ecs']->table('comment').
        " where id_value = '".$row['goods_id']."'" ;
        
         $row['comment_num'] = $GLOBALS['db']->getOne($sql);
        $goods_list[] = $row;
    }
...
function order_fee($order, $goods, $consignee)
{
    ...
    $total['goods_amount'] = $total['amount'];
...
{
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, g.goods_thumb,g.brand_id, g.is_promote, " .
			"g.promote_start_date, g.promote_end_date, g.promote_price, ".
            ...
            "FROM " . $GLOBALS['ecs']->table('cart') ." c LEFT JOIN " . $GLOBALS['ecs']->table('goods') ." g on c.goods_id = g.goods_id " .
            " WHERE c.session_id = '" . SESS_ID . "' " .
            "AND c.rec_type = '$type' order by c.rec_id desc";
    ...
    foreach ($arr as $key => $value)
    {
		...
		if($value['is_gift']>0)
		{
			$m_sql = "SELECT min_amount FROM " . $GLOBALS['ecs']->table('favourable_activity') . " WHERE act_id = ".$value['is_gift'];
    		$m_f = $GLOBALS['db']->getOne($m_sql);
			if(!empty($m_f))
    		{
				$arr[$key]['min_amount'] = $m_f;
			}
		}
		else
		{
			...
			$arr[$key]['min_amount'] = 0;
			if ($value['extension_code'] == 'package_buy')
			{
				$arr[$key]['package_goods_list'] = get_package_goods($value['goods_id']);
			}
			if($value['promote_price']>0 && $value['is_promote'] >0 )
			{
				if ($value['promote_price'] == 0)
				{
					$arr[$key]['is_promote_price'] = 0;
				}
				else
				{
					$time = gmtime();
					if ($time >= $value['promote_start_date'] && $time <= $value['promote_end_date'])
					{
						$arr[$key]['is_promote_price'] = $value['promote_price'];
					}
					else
					{
						$arr[$key]['is_promote_price'] = 0;
		...
		if(empty($value['goods_thumb']))
		{
			$arr[$key]['goods_thumb'] = "images/no_picture.gif";
		}
    }
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    {
        //检查：商品购买数量是否大于总库存
        if ($num > $goods['goods_number']||$goods['goods_number'] < 1)
        {
        ...
        if (is_spec($spec) && !empty($prod))
        {
            ...
            {
                /* 取规格的货品库存 */
                if ($num > $product_info['product_number']||$product_info['product_number'] < 1)
                {
...
function clear_cart($type = CART_GENERAL_GOODS)
{
    ...
    $sql2="select distinct(c.goods_id) from ".$GLOBALS['ecs']->table('cart')." as c left join ".$GLOBALS['ecs']->table('goods')." as g on c.goods_id=g.goods_id where g.is_on_sale =1 AND c.user_id=".$_SESSION['user_id'];
...
function get_cart_goods()
{
    ...
    $total = array(
		...
		'favourable_amount' => 0, // 本店售价合计含赠品
    );

    /* 循环、统计 */
    $sql = "SELECT c.*, IF(c.parent_id, c.parent_id, c.goods_id) AS pid ,g.goods_thumb, g.is_promote," .
			" g.promote_start_date, g.promote_end_date, g.promote_price ".
            " FROM " . $GLOBALS['ecs']->table('cart') . " c LEFT JOIN " . $GLOBALS['ecs']->table('goods') ." g on c.goods_id = g.goods_id " .
            " WHERE c.session_id = '" . SESS_ID . "' AND c.rec_type in ('" . CART_GENERAL_GOODS . "','".CART_EXCHANGE_GOODS."')" .
            " ORDER BY pid, c.parent_id, c.is_gift asc";
    $res = $GLOBALS['db']->query($sql);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		if($row['is_gift']<=0&&$row['rec_type']<4)
		{
			$total['goods_price']  += $row['goods_price'] * $row['goods_number'];
			$total['market_price'] += $row['market_price'] * $row['goods_number'];
		}elseif($row['rec_type']==4){
			$_SESSION['extension_code'] = 'exchange_goods';
			$_SESSION['extension_id'] = $row['goods_id'];
			$total['e_goods_price'] += $row['goods_price'];
		}
		if($row['is_promote'] >0 && $row['promote_price'] >0 )
		{
		...
		$row['is_promote_price'] = $promote_price;
		$row['e_goods_price']  = (int)$row['goods_price'];
		...
		if($row['is_gift']>0)
		{
			$m_sql = "SELECT * FROM " . $GLOBALS['ecs']->table('favourable_activity') . " WHERE act_id = ".$row['is_gift'];
			$m_res = $GLOBALS['db']->query($m_sql);
    		while ($favourable = $GLOBALS['db']->fetchRow($m_res))
    		{
...
function order_query_sql($type = 'finished', $alias = '')
{
	...
	if($type=='today_pay')
	{
		$dtime=mktime(0,0,0,date("n",time()),date("d",time()),date("Y",time()));
		return " AND {$alias}add_time > ".$dtime;
	}
	/* 今日已确认订单数量 */
	if($type=='today_confirmation')
	{
		$dtime=mktime(0,0,0,date("n",time()),date("d",time()),date("Y",time()));
		return " AND {$alias}order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED)) .
			   " AND {$alias}add_time > ".$dtime;
	}
	/* 今日取消订单数量 */
	if($type=='today_cancel')
	{
		$dtime=mktime(0,0,0,date("n",time()),date("d",time()),date("Y",time()));
		return " AND {$alias}order_status " . db_create_in(array(OS_CANCELED)) .
			   " AND {$alias}add_time > ".$dtime;
	}
	/* 今日未确认订单数量 */
	if($type=='today_unconfirmation')
	{
		$dtime=mktime(0,0,0,date("n",time()),date("d",time()),date("Y",time()));
	...
	/* 今日注册会员数量 */
	if($type=='today_member')
	{
		$dtime=local_mktime(0,0,0,date("n",time()),date("d",time()),date("Y",time()));
		return " AND reg_time > ".$dtime;
	}
	/* 已完成订单 */
    if ($type == 'guest_finished')
    {
        return " AND {$alias}order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED)). " ";
    }
    /* 已完成订单 */
