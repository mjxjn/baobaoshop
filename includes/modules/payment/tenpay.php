...
class tenpay
{
    ...
    function get_code($order, $payment ,$type='')
    {
        ...
        if(empty($type))
        {
			$button = '<div style="text-align:center" class="user_pay"><a class="onlinebuy_btn" href="https://gw.tenpay.com/gateway/pay.htm?'.$param. '&sign='.$sign.'" target="_blank"><a></div>'; 
        }elseif($type=='order_list'){
        	$button = '<a href="https://gw.tenpay.com/gateway/pay.htm?'.$param. '&sign='.$sign.'" target="_blank">付款</a>';
        }
        return $button;
    }
