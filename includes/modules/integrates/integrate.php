...
class integrate
{
    ...
    function edit_user_pwd($cfg)
    {
        if (empty($cfg['mobile_phone']))
        {
            return false;
        }
        else
        {
            $cfg['post_mobile_phone'] = $cfg['mobile_phone'];
        }
        ...
        if ($values)
        {
            $sql = "UPDATE " . $this->table($this->user_table).
                   " SET " . implode(', ', $values).
                   " WHERE " . $this->field_mobile . "='" . $cfg['post_mobile_phone'] . "' LIMIT 1";
            ...
            if ($this->need_sync)
            {
                if (empty($cfg['md5password']))
                {
                    $this->sync($cfg['mobile_phone']);
                }
                else
                {
                    $this->sync($cfg['post_mobile_phone'], '', $cfg['md5password']);
                }
            }
        }
    ...
    function get_profile_by_name($username)
    {
        ...
        $sql = "SELECT " . $this->field_id . " AS user_id," . $this->field_name . " AS user_name," .
					$this->field_mobile . " AS mobile_phone,".
               ...
               " FROM " . $this->table($this->user_table) .
               " WHERE " .$this->field_name . "='$post_username' or ".$this->field_mobile . "='$post_username' or ".$this->field_email . "='$post_username'";
    ...
    function check_user($username, $password = null)
    {
        ...
        else
        {
            $sql = "SELECT " . $this->field_id .
				   ...
				   " OR (". $this->field_mobile ."='" . $post_username . "' AND " . $this->field_pass . " ='" . $this->compile_password(array('password'=>$password)) . "')".
				   " OR (". $this->field_email ."='" . $post_username . "' AND " . $this->field_pass . " ='" . $this->compile_password(array('password'=>$password)) . "')";
            return  $this->db->getOne($sql);
        }
    }
    ...
    function sync ($username, $password='', $md5password='')
    {
        ...
        $sql = "SELECT user_name, mobile_phone, email, password, sex, birthday".
        ...
        if (empty($profile))
        {
            ...
            if (empty($md5password))
            {
               $sql = "INSERT INTO " . $GLOBALS['ecs']->table('users').
                            "(user_name, mobile_phone, email, sex, birthday, reg_time)".
                      " VALUES('$username', '" .$main_profile['mobile_phone']."','".$main_profile['email']."','".
                            $main_profile['sex'] . "','" . $main_profile['birthday'] . "','" . $main_profile['reg_time'] . "')";
            }
            else
            {
               $sql = "INSERT INTO " . $GLOBALS['ecs']->table('users').
                            "(user_name,mobile_phone, email, sex, birthday, reg_time, password)".
                      " VALUES('$username', '" .$main_profile['mobile_phone']."','".$main_profile['email']."','".
