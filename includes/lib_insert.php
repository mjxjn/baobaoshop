...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
			...
			$str .='<li>
						<div class="r_pic"><a href="'.$goods['url'].'"><img src="'.$goods['goods_thumb'].'" width="83" height="83" alt="'.$goods['goods_name'].'"/></a></div>
						<div class="r_infos">
							<div class="r_name">
								<a href="'.$goods['url'].'" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a>
							</div>
							<div class="r_price">
								婴格价：<span class=red12>'.$goods['shop_price'].'</span>
							</div>
							<div class="r_btn">
								<a href="javascript:addToCart('.$goods['goods_id'].')"><img src="themes/yingge/images/r_btn.jpg" /></a>
							</div>
						</div>
					</li>';
        }
...
function insert_user_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
			...
			$str.='<dd><a target="_blank" href="'.$goods['url'].'" title="'.$goods['goods_name'].'"><img src="'.$goods['goods_thumb'].'"></a></dd>';
        }
        //$str .= '<ul id="clear_history"><a onclick="clear_history()">' . $GLOBALS['_LANG']['clear_history'] . '</a></ul>';
    }
    return $str;
...
function insert_cart_info()
{
    ...
    else
    {
    	...
    	$str = sprintf($GLOBALS['_LANG']['cart_info_top'], $number, price_format($amount, false));
 ...
 * @return  string
 */
function insert_cart_info_activity()
{
    $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') .
    ...
    else
    {
    	...
    	$str = sprintf($GLOBALS['_LANG']['cart_info_activity'], price_format($amount, false));
...
function insert_ads($arr)
{
    ...
    foreach ($res AS $row)
    {
        ...
        {
            case 0: // 图片广告
            	//define('DATA_DIR', "/data/afficheimg");
 ...
 * @return  string
 */
function insert_baby_member_info()
{
    $need_cache = $GLOBALS['smarty']->caching;
    $GLOBALS['smarty']->caching = false;
    ...
    $output = $GLOBALS['smarty']->fetch('starbaby/library/baby_member_info.lbi');
 ...
 * @return  string
 */
function insert_usercenter_member_info()
{
	$need_cache = $GLOBALS['smarty']->caching;
	$GLOBALS['smarty']->caching = false;
	...
	$output = $GLOBALS['smarty']->fetch('user/library/usercenter_member_info.lbi');
...
function insert_index_user_info()
{
    ...
    $output = $GLOBALS['smarty']->fetch('library/index_user_info.lbi');
...
function insert_comments($arr)
{
    ...
    $GLOBALS['smarty']->assign('rank',        $cmt['rank']);
    $GLOBALS['smarty']->assign('goods_name',   goods_name($arr['id']));
 ...
 * @return  string
 */
function insert_baby_comments($arr)
{
    $need_cache = $GLOBALS['smarty']->caching;
    $need_compile = $GLOBALS['smarty']->force_compile;
    ...
    $GLOBALS['smarty']->assign('comment_type', $arr['type']);
    $GLOBALS['smarty']->assign('id',           $arr['id']);
    $cmt = assign_baby_comment($arr['id'],     $arr['type']);
    $GLOBALS['smarty']->assign('comments',     $cmt['comments']);
    $GLOBALS['smarty']->assign('pager',        $cmt['pager']);


    $val = $GLOBALS['smarty']->fetch('starbaby/library/baby_comments_list.lbi');
