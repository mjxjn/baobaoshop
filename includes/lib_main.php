...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    if ($filename != 'index')
    {
        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
					...
					$channel_name = htmlspecialchars($val['cat_name']);
                    $args       = array($key => $val['cat_id']);
                    $ur_here   .= ' <span>»</span> <span class="now"><a href="' . build_uri($type, $args, $val['cat_name']) . '">' .
                                    htmlspecialchars($val['cat_name']) . '</a></span>';
                }
            }
        }
        /* 处理无分类的 */
        else
        {
            ...
            if ('brand' == $filename)
            {
                $page_title = '品牌中心_' . $page_title;
                //$args       = array('gbid' => '0');
                $ur_here   .= ' <span>»</span> <a href="brand.php">品牌中心</a>';
            }
            /* 团购 */
            if ('group_buy' == $filename)
            {
                ...
                $args       = array('gbid' => '0');
                $ur_here   .= ' <span>»</span> <a href="group_buy.php">' .
            ...
            elseif ('auction' == $filename)
            {
                ...
                $args       = array('auid' => '0');
                $ur_here   .= ' <span>»</span> <a href="auction.php">' .
            ...
            elseif ('snatch' == $filename)
            {
                ...
                $args       = array('id' => '0');
                $ur_here   .= ' <span>»</span><a href="snatch.php">' . $GLOBALS['_LANG']['snatch_list'] . '</a>';
            }
            /* 批发 */
            elseif ('wholesale' == $filename)
            {
                ...
                $args       = array('wsid' => '0');
                $ur_here   .= ' <span>»</span> <a href="wholesale.php">' .
            ...
            elseif ('exchange' == $filename)
            {
                ...
                $args       = array('wsid' => '0');
                $ur_here   .= ' <span>»</span> <a href="exchange.php">积分商城</a>';
            }
            /* 积分兑换 */
            elseif ('integral' == $filename)
            {
            	$page_title = $GLOBALS['_LANG']['exchange'] . '_' . $page_title;
            	$args       = array('wsid' => '0');
            	$ur_here   .= ' <span>»</span> <a href="integral.php">积分商城</a>';
            }
            /* 其他的在这里补充 */
			/* 即时促销 */
			elseif('promotion' == $filename)
			{
				$page_title = $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <span>»</span> <a href="promotion.php">特价促销</a>';
			}
			elseif('user' == $filename)
			{
				$args       = array('wsid' => '0');
				$ur_here   .= ' <span>»</span> <a href="user.php">用户中心</a>';
			}
        }
    }
    ...
    if (!empty($str))
    {
        ...
        $ur_here    .= ' <span>»</span> ' . $str;
    }

    /* 返回值 */
    return array('title' => $page_title, 'ur_here' => $ur_here, 'channel_name' => $channel_name);
}
                        ...
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='')
{
    ...
    switch ($app)
    {
        ...
        case 'brand':
            ...
            break;
		case 'brand_list':
            $uri_args = array('cid' => 0, 'bid' => 0, 'sort' => $sort, 'order' => $order, 'display' => $display_type);
            break;
        case 'search':
...
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
	...
	$GLOBALS['smarty']->assign('channel_name', $position['channel_name']); // 当前栏目名
 ...
 * @return  void
 */
function show_starbaby_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
    assign_template();
    ...
    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->display('starbaby/baby_message.htm');
...
function assign_comment($id, $type, $page = 1)
{
    ...
    $sum_ping = $GLOBALS['db']->getOne('SELECT SUM(comment_rank) FROM ' .$GLOBALS['ecs']->table('comment').
           " WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0");
    if($count){
    	$rank=number_format(($sum_ping/$count)*10,0);
    }else{
    	$rank=100;
    }
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['comment_id']]['rank']     = ($row['comment_rank']*10);
    ...
    $cmt = array('comments' => $arr, 'pager' => $pager , 'rank' => $rank);
 ...
 * @return  array
 */
function assign_baby_comment($id, $type, $page = 1)
{
    /* 取得评论列表 */
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           " WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0");
    ...
    $pager['record_count'] = $count;
    $pager['page_count']   = $page_count;
    $pager['page_first']   = "javascript:gotoBabyPage(1,$id,$type)";
    $pager['page_prev']    = $page > 1 ? "javascript:gotoBabyPage(" .($page-1). ",$id,$type)" : 'javascript:;';
    $pager['page_next']    = $page < $page_count ? 'javascript:gotoBabyPage(' .($page + 1) . ",$id,$type)" : 'javascript:;';
    $pager['page_last']    = $page < $page_count ? 'javascript:gotoBabyPage(' .$page_count. ",$id,$type)"  : 'javascript:;';
