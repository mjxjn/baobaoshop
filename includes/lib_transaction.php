...
function get_profile($user_id)
{
    ...
    $sql  = "SELECT user_name, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer, is_validated  ".
 ...
 */
function get_user_orders($user_id, $num = 10, $start = 0, $paystatus = '', $shippingstatus = '', $orderstatus = '')
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    if($shippingstatus===1){
    	$where = " shipping_status in (1,3,4,5)";
    }
    if($orderstatus===1){
    	$where = " order_status in (5)";
    }

    $sql = "SELECT order_id, order_sn, consignee, pay_name, order_status, shipping_status, pay_status,pay_id, add_time,extension_code, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
           " FROM " .$GLOBALS['ecs']->table('order_info') .
           " WHERE user_id = '$user_id' and $where ORDER BY add_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);

    while ($row = $GLOBALS['db']->fetchRow($res))
    {
    	$sql="select is_cod from ".$GLOBALS['ecs']->table('payment') .
    	" where pay_id = ".$row['pay_id'];
    	...
    	if($is_cod==1){
    		if($row['order_status'] == OS_CANCELED){
	    		...
	    		if($row['order_status']==OS_UNCONFIRMED){
	    			$row['order_status'] = "等待配货";
	    			$row['handler']="<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. "\">查看</a>&nbsp;|&nbsp;<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\" target='_blank'>".$GLOBALS['_LANG']['cancel']."</a>";
	    		}
	    		else if($row['order_status'] == OS_CONFIRMED){
	    			$row['order_status'] = "开始配货";
	    			$row['handler']="<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. "\">查看</a>";
	    		}
	    		else if ($row['order_status'] == OS_SPLITED)
	        	{
	           	 	/* 对配送状态的处理 */
	            	if ($row['shipping_status'] == SS_SHIPPED || $row['shipping_status'] ==SS_SHIPPED_ING || $row['shipping_status'] ==SS_PREPARING || $row['shipping_status'] ==SS_SHIPPED_PART)
	            	{
	            		$row['order_status'] = "已发货";
	               		@$row['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\" target='_blank'>".$GLOBALS['_LANG']['received']."</a>&nbsp;|&nbsp;". '<a href="user.php?act=order_detail&order_id=' .$row['order_id']. "\">查看</a>";
	            	}
    		...
    		if($row['order_status'] == OS_CANCELED){
	    		...
	    		if ($row['pay_status'] == PS_UNPAYED)
	    		{
	    			...
	    			$row['pay_online'] = $pay_obj->get_code($row, $payment,'order_list');
	    			@$row['handler'] = $row['pay_online']. '&nbsp;|&nbsp;'.'<a href="user.php?act=order_detail&order_id=' .$row['order_id']. '">查看</a>&nbsp;|&nbsp;'."<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\" target='_blank'>".$GLOBALS['_LANG']['cancel']."</a>";
	    			//}else{
	    				//@$row['handler'] ='<a href="user.php?act=order_detail&order_id=' .$row['order_id']. '">查看</a>';
	    			//}
	    		}
        ...
        /*if ($row['order_status'] == OS_UNCONFIRMED)
        {
            $row['handler'] = "<a href=\"user.php?act=cancel_order&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\" target='_blank'>".$GLOBALS['_LANG']['cancel']."</a>";
        }
        else if ($row['order_status'] == OS_SPLITED)
        {
         */   /* 对配送状态的处理 */
          /*  if ($row['shipping_status'] == SS_SHIPPED)
            {
                @$row['handler'] = "<a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\" target='_blank'>".$GLOBALS['_LANG']['received']."</a>";
            }
            elseif ($row['shipping_status'] == SS_RECEIVED)
            {
        ...
        $arr[] = array('order_id'       => $row['order_id'],
					   ...
					   'order_time'     => date('Y-m-d', $row['add_time']),
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT o.user_id, o.order_sn , o.order_status, o.shipping_status, o.pay_status, o.trade_no, d.shipping_id FROM ".$GLOBALS['ecs']->table('order_info') ." as o left join ".
	$GLOBALS['ecs']->table('delivery_order') ." as d on o.order_sn=d.order_sn".
	" WHERE o.order_id = '$order_id'";

    $order = $GLOBALS['db']->GetRow($sql);
	
	//$sql= "select * from ".$GLOBALS['ecs']->table('payment')." where pay_id=".$order['shipping_id'];
    ...
    else
    {
		...
		$sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET shipping_status = '" . SS_RECEIVED . "', pay_status ='".PS_PAYED."' WHERE order_id = '$order_id'";

        if ($GLOBALS['db']->query($sql))
        {
            /* 记录日志 */
            order_action($order['order_sn'], $order['order_status'], SS_RECEIVED, PS_PAYED, '', $GLOBALS['_LANG']['buyer']);
			
			/*if($payment['pay_code']=='alipay'){
				ecs_header("Location: http://www.alipay.com\n");
				exit;
			}*/
            return true;
        }
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    $order = order_info_user($order_id);
    ...
    if ($order['pay_time'] > 0 && $order['pay_status'] != PS_UNPAYED)
    {
    	...
    	$order['pay_time']=local_date('Y-m-d h:i:s',$order['pay_time']);
    }
    else
    {
        $order['pay_time'] = '';
    }
    if ($order['shipping_time'] > 0 && in_array($order['shipping_status'], array(SS_SHIPPED, SS_RECEIVED,SS_PREPARING)))
    {
        //$order['shipping_time'] = sprintf($GLOBALS['_LANG']['shipping_time'], local_date($GLOBALS['_CFG']['time_format'], $order['shipping_time']));
        $order['shipping_time']=local_date('Y-m-d h:i:s',$order['shipping_time']);
    }
    else
    {
        $order['shipping_time'] = '';
