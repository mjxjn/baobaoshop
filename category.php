...
$price_code = isset($_REQUEST['price_code']) && intval($_REQUEST['price_code']);
...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'sales'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
	...
	$smarty->assign('channel_name',          $position['channel_name']);  // 当前位置
	...
	$smarty->assign('price_code',        $price_code);
