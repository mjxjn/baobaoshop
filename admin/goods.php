...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    foreach($goods_list['goods'] as $key => $val){
    	$sql = "SELECT brand_name FROM " . $ecs->table('brand') . " WHERE brand_id = '".$val[brand_id]."'";
        $brand_name = $db->getRow($sql);
    	$goods_list['goods'][$key]['brand_name']=$brand_name['brand_name'];
    	$sql = "SELECT cat_name FROM " . $ecs->table('category') . " WHERE cat_id = '".$val[cat_id]."'";
        $cat_name = $db->getRow($sql);
    	$goods_list['goods'][$key]['cat_name']=$cat_name['cat_name'];
    }
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /* 处理商品数据 */
	$goods_num   = isset($_POST['goods_num']) ? $_POST['goods_num'] : '';
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, goods_num, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$goods_num', '$catgory_id', " .
...
//-- 修改商品货号
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_goods_num')
{
    check_authz_json('goods_manage');

    $goods_id = intval($_POST['id']);
    $goods_num = json_str_iconv(trim($_POST['val']));

    /* 检查是否重复 */
    if (!$exc->is_only('goods_num', $goods_num, $goods_id))
    {
        make_json_error($_LANG['goods_num_exists']);
    }

    if ($exc->edit("goods_num = '$goods_num', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result(stripslashes($goods_num));
    }
}
...
elseif ($_REQUEST['act'] == 'product_list')
{
	...
	$smarty->assign('m_sql', $product['m_sql']);
