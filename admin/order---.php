    ...
    if ('confirm' == $operation)
    {
        foreach($order_id_list as $id_order)
        {
            ...
            if($order)
            {
                ...
                /* 标记订单为已确认 */
                update_order($order_id, array('order_status' => OS_CONFIRMED,'pay_status'=>PS_PAYED, 'confirm_time' => gmtime()));
                update_order_amount($order_id);
...
function operable_list($order)
{
    ...
    if (OS_UNCONFIRMED == $os)
    {
        ...
        /*if ($priv_list['os'])
        {*/
            $list['confirm']    = true; // 确认订单
            $list['invalid']    = false;//true; // 无效
            $list['cancel']     = true; // 确认取消
            ...
            /*if ($is_cod||$is_bank)
            {
              ...
              /*   if ($priv_list['ss'])
                {
                    $list['prepare'] = false;//true; // 配货
                    $list['split'] = false;//true; // 分单
                }
            }
            else
            {
            ...
            /*     if ($priv_list['ps'])
                {
                    $list['pay'] = false;//true;  // 付款
                }
            }
        }*/
    }
    elseif(OS_CONFIRMED == $os&&PS_PAYED == $ps){
    	$list['prepare'] = true;//配货
    	$list['cancel'] = true; // 取消
    }
    elseif(SS_PREPARING==$ss){
