...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    /* 今日订单总数 */
    $order['today_pay']   =$db->GetOne('SELECT COUNT(*) FROM '. $ecs->table('order_info').
    " WHERE 1 " . order_query_sql('today_pay'));
    
    /* 今日已确认订单数量 */
    $order['today_confirmation']  =$db->GetOne('SELECT COUNT(*) FROM '. $ecs->table('order_info').
    " WHERE 1 " . order_query_sql('today_confirmation'));
    
    /* 今日已确认订单数量 */
    $order['today_cancel'] =$db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('order_info').
    " WHERE 1 " . order_query_sql('today_cancel'));
    
    /* 今日未确认订单数量 */
    $order['today_unconfirmation'] =$db->GetOne('SELECT COUNT(*) FROM '. $ecs->table('order_info').
    " WHERE 1 " . order_query_sql('today_unconfirmation'));

	    /* 今日订单总数 */
    $order['today_money'] =$db->GetOne('SELECT sum(goods_amount) FROM '. $ecs->table('order_info').
    " WHERE 1 " . order_query_sql('today_money'));
    if($order['today_money']==0)
    {
    	$order['today_money']=0;
    }
    
    /* 今日注册会员数量 */
    $order['today_member'] =$db->GetOne('SELECT COUNT(*) FROM '. $ecs->table('users').
    " WHERE 1 " . order_query_sql('today_member'));
    
    /* 今日平均客单价 */
    if($order['today_pay']==0||($order['today_pay']-$order['today_cancel'])==0)
    {
    	$order['today_vm']=0;
    }else{
    	$order['today_vm'] = $order['today_money']/($order['today_pay']-$order['today_cancel']);
    }
...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    //$nowtime -= 86400*3;//3天
    $sql="SELECT order_id,user_id,shipping_time FROM ". $ecs->table('order_info') . " WHERE order_status ='".OS_SPLITED."' and shipping_status <> '" . SS_RECEIVED . "' and shipping_time <>0 and shipping_time<".$nowtime;
    $row = $db->GetAll($sql);
    
    foreach ($row as $rs){
    	if($nowtime-($rs['shipping_time']+86400*3)>0){
    		affirm_received($rs['order_id'],$rs['user_id']);
    	}
    }
