...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('baby_comment_list.htm');
}
...
function get_comment_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('comment'). " WHERE parent_id = 0 AND comment_type like '11%' $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('comment'). " WHERE parent_id = 0 AND comment_type like '11%' $where " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $sql = ($row['comment_type'] == 0) ?
            ...
            "SELECT baby_name FROM ".$GLOBALS['ecs']->table('baby_baby'). " WHERE baby_id='$row[id_value]'";
