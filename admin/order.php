...
/* elseif($_REQUEST['act'] == 'mx'){
	...
	$sql="select order_id from ". $GLOBALS['ecs']->table('order_info') ." where order_status=1 and shipping_status=1 order by order_id desc limit 0,610";//610
	...
	foreach($orderinfo as $val){
		...
		$delivery_stock_sql = "SELECT og.goods_id, og.is_real, og.goods_number
		FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og, " . $GLOBALS['ecs']->table('order_info')
		. " AS oi WHERE og.order_id = oi.order_id AND oi.order_id = '$order_id' GROUP BY oi.order_id ";
		$delivery_stock_result = $GLOBALS['db']->getAll($delivery_stock_sql);

		foreach ($delivery_stock_result as $value)
		{
			$minus_stock_sql = "UPDATE " . $GLOBALS['ecs']->table('goods') . " SET goods_number = goods_number - " . $value['goods_number'] . " WHERE goods_id = " . $value['goods_id'];
		...
		$user = user_info($order['user_id']);// 取得用户信息
		//var_dump($user);
		//var_dump($delivery_stock_result);
		//var_dump($order);
		
		$integral = integral_to_give($order);// 计算并发放积分 
		var_dump($integral);
	 log_account_change($order['user_id'], 0, 0, intval($integral['rank_points']), intval($integral['custom_points']), sprintf($_LANG['order_gift_integral'], $order['order_sn']));

	 
	 send_order_bonus($order_id);// 发放红包 
	}
} */
...
elseif ($_REQUEST['act'] == 'operate_post')
{
	...
	/* 订单已完成 */
	if ('completed' == $_REQUEST['operation'])
	{
		
		/* 检查权限 */
        admin_priv('order_ps_edit');
		
		/* 标记订单为已分单，更新付款时间和已支付金额，如果是货到付款，同时修改订单为“收货确认” */
        if ($order['order_status'] != OS_SPLITED)
        {
            $arr['order_status']    = OS_SPLITED;
            $arr['confirm_time']    = gmtime();
        }
		 if ($arr['pay_status'] != PS_PAYED){
		 	$arr['pay_status']  = PS_PAYED;
			$arr['pay_time']    = gmtime();
		 }
        ...
        $payment = payment_info($order['pay_id']);
           ...
           if($arr['shipping_status'] != SS_RECEIVED){
		...
		update_order($order_id, $arr);
		/* 记录log */
        order_action($order['order_sn'], OS_SPLITED, SS_RECEIVED, PS_PAYED, $action_note);
		/* 记录log */
        /*order_action($order['order_sn'], OS_CONFIRMED, $order['shipping_status'], PS_PAYED, $action_note);*/
	}
    /* 确认 */
    elseif ('confirm' == $operation)
    {
        /* 标记订单为已确认 */
        update_order($order_id, array('order_status' => OS_CONFIRMED, 'confirm_time' => gmtime()));
        update_order_amount($order_id);
		/* 获取手机号 */
		$sql="select u.mobile_phone from ".$GLOBALS['ecs']->table('users') ." u left join ".$GLOBALS['ecs']->table('order_info')." o on o.user_id=u.user_id where o.order_id=".$order_id;
		...
		$sql = 'SELECT `serialNumber`, `password`, `smsnum`, `sessionKey`, `confirm_charge` 
		...
		*/	
		...
		$send_num=
		$msg="尊敬的婴格会员，您的订单（订单号：".$order['order_sn']."）已经处理。".$sms_site_info['confirm_charge'];
