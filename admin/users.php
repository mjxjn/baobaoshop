...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	$pay_points=empty($_POST['pay_points']) ? 0 : trim($_POST['pay_points']);
    $users =& init_users();
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['keywords'])
        {
            $ex_where .= " AND user_name LIKE '%" . mysql_like_quote($filter['keywords']) ."%' or mobile_phone LIKE '%".mysql_like_quote($filter['keywords'])."%'";
        }
        if ($filter['rank'])
        {
        ...
        $sql = "SELECT user_id, user_name, email,mobile_phone, keynumber,baby,babybirthday, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
    ...
    for ($i=0; $i<$count; $i++)
    {
		...
		if($user_list[$i]['baby']==1)
		{
			$user_list[$i]['baby']="男宝宝";
		}elseif($user_list[$i]['baby']==2)
		{
			$user_list[$i]['baby']="女宝宝";
		}elseif($user_list[$i]['baby']==3){
			$user_list[$i]['baby']="预产期";
		}else{
			$user_list[$i]['baby']="";
		}
    }
