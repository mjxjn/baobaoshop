...
{
    $smarty->assign('ur_here',      $_LANG['16_double_integral']);
    $smarty->assign('action_link',  array('text' => $_LANG['integral_add'], 'href'=>'double_integral.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('double_integral_list.htm');
}
...
function get_packagelist()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods_activity') .
               " WHERE act_type =" . GAT_INTEGRAL . $where;
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $filter = page_and_size($filter);

        /* 获活动数据 */
        $sql = "SELECT act_id, act_name AS integral_name, start_time, end_time, is_finished, ext_info ".
    ...
    $arr = array('integral' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
