...
if ($_REQUEST['act']=='reply')
{
	...
	elseif($comment_info['comment_type'] == 11){
		$sql = "SELECT baby_name FROM ".$ecs->table('baby_baby').
               " WHERE baby_id='$comment_info[id_value]'";
        $id_value = $db->getOne($sql);
	}
    else
    {
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql="select id_value from ".$ecs->table('comment'). " WHERE comment_id = '$id'";
    $id_value=$db->getOne($sql);
    
    
		$sql = "select pay_points from " .$GLOBALS['ecs']->table('account_log')." where change_type=77 and user_id=".$user_id." order by log_id desc limit 0,1";
		$logpay_points = $db->getOne($sql);
		if($logpay_points>0){
		$sql = "select pay_points from ".$GLOBALS['ecs']->table('users'). " where user_id=".$user_id;
		$pay_points = $db->getOne($sql);
		$pay_points -= $logpay_points;
		...
		"VALUES ('$user_id', '-$logpay_points', '$nowdate','删除商品评论(前10条)','77')";
		$db->query($sql);
		}else{
							...
							$db->query($sql);
							$sql="INSERT INTO " .$GLOBALS['ecs']->table('account_log')." (user_id,pay_points,change_time,change_desc,change_type)".
							"VALUES ('$user_col[$i]', '-5', '$nowdate','删除商品评论','77')";
							$db->query($sql);
		}
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            case 'remove':
		        $sql = "SELECT user_id FROM " . $ecs->table('comment') . " WHERE ".db_create_in($_POST['checkboxes'], 'comment_id');
		        $user_col = $db->getCol($sql);
		        $userid = implode(',',addslashes_deep($user_col));
		        $user_count = count($user_col);
		        
		        $sql = "SELECT id_value FROM " . $ecs->table('comment') . " WHERE ".db_create_in($_POST['checkboxes'], 'comment_id');
		        $col = $db->getCol($sql);
		        $id_value = implode(',',addslashes_deep($col));
		        $count = count($col);
				...
				if($user_count!=0){
					...
					for($i=0;$i<$user_count;$i++){
						...
						$sql = "select pay_points from " .$GLOBALS['ecs']->table('account_log')." where change_type=77 and user_id=".$user_col[$i]." order by log_id desc limit 0,1";
						$logpay_points = $db->getOne($sql);
						if($logpay_points>0)
						{
							$sql = "select pay_points from ".$GLOBALS['ecs']->table('users'). " where user_id=".$user_col[$i];
							...
							$db->query($sql);
							$sql="INSERT INTO " .$GLOBALS['ecs']->table('account_log')." (user_id,pay_points,change_time,change_desc,change_type)".
							"VALUES ('$user_col[$i]', '-$logpay_points', '$nowdate','删除商品评论(前10条)','77')";
							$db->query($sql);
						}else{
							$sql = "select pay_points from ".$GLOBALS['ecs']->table('users'). " where user_id=".$user_col[$i];
							$pay_points = $db->getOne($sql);
...
function get_comment_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('comment'). " WHERE parent_id = 0 AND comment_type=0 $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('comment'). " WHERE parent_id = 0 AND comment_type=0 $where " .
