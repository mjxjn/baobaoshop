...
elseif($_REQUEST['act'] == 'excel')
{
	...
	/* 显示模板 */
    assign_query_info();
    $smarty->display('goods_batch_excel.htm');
}
...
elseif ($_REQUEST['act'] == 'excel_upload')
{
    /* 检查权限 */
    admin_priv('goods_batch');
    ...
    echo "<script>alert('共有".$total."条数据，更新其中".$counts."条数据成功'); location='goods_batch.php?act=excel'</script>";
