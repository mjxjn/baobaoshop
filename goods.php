...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
		...
		$linked_price_all=0;
		$linked_mkprice_all=0;
		foreach( $linked_goods as $k => $v)
		{
			if($v[$k]['promote_price'] > 0)
			{
				$linked_price_all+=$v['promote_price'];
			}else
			{
				$linked_price_all+=$v['mshop_price'];
			}
			$linked_mkprice_all+=$v['mmarket_price'];
			$linked_goods_name.=$v['goods_name']."+";
		}
		if($goods['is_promote'] !=0 && $goods['gmt_end_time'] !=0)
		{
			$linked_price_all+=$goods['promote_price_org'];
		}else{
			$linked_price_all+=$shop_price;
		}
		$linked_mkprice_all+=$goods['mmarket_price'];
		$smarty->assign('linked_goods_name',             substr($linked_goods_name,0,-1));
		$smarty->assign('linked_price_all',              $linked_price_all);
		$smarty->assign('linked_mkprice_all',              $linked_mkprice_all);
		$smarty->assign('linked_price_all_format',              price_format($linked_price_all));
		$smarty->assign('linked_mkprice_all_format',              price_format($linked_mkprice_all));
		...
		$smarty->assign('mgmt_end_time',local_date('D, d M Y H:i:s O', $goods['gmt_end_time']));
		...
		$smarty->assign('related_goods_conut', sizeof($linked_goods));                        // 关联商品数量
		$smarty->assign('related_goods_ul',    sizeof($linked_goods)*102+90);
...
function get_linked_goods($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['mmarket_price'] = $row['market_price'];
        $arr[$row['goods_id']]['mshop_price']   = $row['shop_price'];
