...
{
	if($cmt->type==11){
    	$comments = assign_baby_comment($cmt->id, $cmt->type, $cmt->page);
	}else{
    ...
    $smarty->assign('rank',         $comments['rank']);
    ...
    if($cmt->type==11){
    	$result['content'] = $smarty->fetch("starbaby/library/baby_comments_list.lbi");
    }else{
...
function add_comment($cmt)
{
    ...
    $sql="select count(*) as pingnum from ".$GLOBALS['ecs']->table('comment') .
    	 " where id_value=".$cmt->id." and comment_type = ".$cmt->type." AND status = 1 AND parent_id = 0";
	$pingnum=$GLOBALS['db']->getOne($sql);
	$sql="select count(*) as usernum from ".$GLOBALS['ecs']->table('comment') .
    	 " where user_id=".$user_id." and comment_type = ".$cmt->type." AND status = 1 AND parent_id = 0";
	...
	if($pingnum<5){
		if($usernum<1){
			...
			"VALUES ('$user_id', '10', '$nowdate','商品评论前10条','77')";
			$GLOBALS['db']->query($sql);
		}
	}else{
		if($usernum<1){
			...
			$nowdate=time();
			$sql="INSERT INTO " .$GLOBALS['ecs']->table('account_log')." (user_id,pay_points,change_time,change_desc,change_type)".
			"VALUES ('$user_id', '5', '$nowdate','商品评论','77')";
			$GLOBALS['db']->query($sql);
		}
	}
    /* 保存评论内容 */
