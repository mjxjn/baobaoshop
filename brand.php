...
if (empty($brand_id)||$brand_id==0)
{
    ...
    /* 缓存编号 */
    $cache_id = sprintf('%X',crc32($sort . '-' . $order . '-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' . $_CFG['lang'] . '-' . $cate));
    if (!$smarty->is_cached('brand_list.dwt', $cache_id))
    {
        ...
        $smarty->assign('brand_list', brand_get_brand($size, $page, $sort, $order));

		assign_pager('brand_list', $cate, $count, $size, $sort, $order, $page, '', 0, 0, 0, 'grid'); // 分页
...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('sales', 'goods_id', 'shop_price'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
//  print_r ( $brand_info['brand_name']).'</br>';
 ...
 * @return  array
*/
function brand_get_brand($size, $page, $sort, $order)
{
    $startpage=($page - 1) * $size;
    $sql = 'SELECT * ' .
            'FROM ' . $GLOBALS['ecs']->table('brand') .
            " WHERE is_show = 1 ".
            "ORDER BY $sort $order limit $startpage , $size";
    //echo $sql;exit;
    ...
    {
        $row[$key]['url'] = build_uri('brand', array('cid' => 0, 'bid' => $val['brand_id']), $val['brand_name']);
